...
config.DATABASE_URL = 'bolt://neo4j:w1tness1@localhost:7687'  # default


class Uti(StructuredNode):
    _primarykey = 'name'
    _name = 'com.base'

    name = StringProperty()
    inherits = RelationshipTo('Uti', 'inherits')
...
class Version(GraphObject):
    ...
    def __init__(self, product, previous_version=None):
        """
        Args:
...
def more_versions():
    ...
    texture_product.add_version()
    texture_product.add_version()
    for product in texture_product.dependency:
